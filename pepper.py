...
class VoiceHandler(object):
    def __init__(self, session):
        ...
        # PIDs der Prozesse
        self.calculator_pid = None
        self.clock_pid = None
        self.connectfour_pid = None
        self.hangman_pid = None
        self.memory_pid = None
        self.numberguessing_pid = None
        self.quiz_pid = None
        self.rockpaperscissors_pid = None
        self.tictactoe_pid = None
        self.wordpuzzle = None

        self.music_files = [
            "/home/nao/voice_handler/songs/song1.mp3",
            "/home/nao/voice_handler/songs/song2.mp3",
            "/home/nao/voice_handler/songs/song3.mp3",
            "/home/nao/voice_handler/songs/song4.mp3",
            "/home/nao/voice_handler/songs/song5.mp3",
            "/home/nao/voice_handler/songs/song6.mp3",
            "/home/nao/voice_handler/songs/song7.mp3",
            "/home/nao/voice_handler/songs/song8.mp3",
            "/home/nao/voice_handler/songs/song9.mp3",
            "/home/nao/voice_handler/songs/song10.mp3"
        ]

        self.image_files = [
            "/home/nao/voice_handler/images/bild1.png",
            "/home/nao/voice_handler/images/bild2.png",
            "/home/nao/voice_handler/images/bild3.png",
            "/home/nao/voice_handler/images/bild4.png"
        ]

        self.video_files = [
            "/home/nao/voice_handler/videos/video1.mp4",
            "/home/nao/voice_handler/videos/video2.mp4",
            "/home/nao/voice_handler/videos/video3.mp4",
            "/home/nao/voice_handler/videos/video4.mp4"
        ]
    ...
    def play_video(self):
        ...
        video_file_path = random.choice(self.video_files)
        
        # Musikdatei abspielen
        self.tablet_service.playVideo(video_file_path)
    ...
    def stop_video(self):
        try:
            ...
            print("Video gestoppt.")
        except Exception as e:
            print("Fehler beim Stoppen des Videos:", e)
    ...
    def stop_music(self):
        try:
            ...
            print("Fehler beim Stoppen der Musik:", e)
    ...
    def on_speech_recognized(self, value):
        ...
        if confidence > 0.5:
            ...
            # Fortsetzung der Interaktionen und Antworten ...
                ...
                self.play_music()
            elif "Kannst du Musik abspielen?" in recognized_words:
                self.tts_service.say("Natürlich! Hier ist ein Lied für dich.")
                self.play_music()
            elif "Musikwiedergabe beenden" in recognized_words:
                self.stop_music()
            elif "Kannst du mir ein Video zeigen?" in recognized_words:
                self.tts_service.say("Natürlich! Hier ist ein Video für dich.")
                self.play_video()
            elif "Videowiedergabe beenden" in recognized_words:
                self.stop_video()
                ...
                if self.tictactoe_pid:
                    ...
                    self.tictactoe_pid = None
            elif "Taschenrechner starten" in recognized_words:
                # Starte das Calculaotr-Skript und speichere die Prozess-ID (PID)
                calculator_process = subprocess.Popen(["python", "calculator.py"])

                # Holen der PID des gestarteten Prozesses
                self.calculator_pid = calculator_process.pid
            elif "Taschenrechner beenden" in recognized_words:
                if self.calculator_pid:
                    os.kill(self.calculator_pid, 9)
                    self.calculator_pid = None
            elif "Vier Gewinnt starten" in recognized_words:
                # Starte das Connect Four-Skript und speichere die Prozess-ID (PID)
                connectfour_process = subprocess.Popen(["python", "connectfour.py"])

                # Holen der PID des gestarteten Prozesses
                self.connectfour_pid = connectfour_process.pid
            elif "Vier Gewinnt beenden" in recognized_words:
                if self.connectfour_pid:
                    os.kill(self.connectfour_pid, 9)
                    self.connectfour_pid = None
            elif "Digitaluhr starten" in recognized_words:
                # Starte das Clock-Skript und speichere die Prozess-ID (PID)
                clock_process = subprocess.Popen(["python", "clock.py"])

                # Holen der PID des gestarteten Prozesses
                self.clock_pid = clock_process.pid
            elif "Digitaluhr beenden" in recognized_words:
                if self.clock_pid:
                    os.kill(self.clock_pid, 9)
                    self.clock_pid = None
            elif "Galgenmännchen starten" in recognized_words:
                # Starte das Hangman-Skript und speichere die Prozess-ID (PID)
                hangman_process = subprocess.Popen(["python", "hangman.py"])

                # Holen der PID des gestarteten Prozesses
                self.hangman_pid = hangman_process.pid
            elif "Galgenmännchen beenden" in recognized_words:
                if self.hangman_pid:
                    os.kill(self.hangman_pid, 9)
                    self.hangman_pid = None
            elif "Memory starten" in recognized_words:
                # Starte das Memory-Skript und speichere die Prozess-ID (PID)
                memory_process = subprocess.Popen(["python", "memory.py"])

                # Holen der PID des gestarteten Prozesses
                self.memory_pid = memory_process.pid
            elif "Memory beenden" in recognized_words:
                if self.memory_pid:
                    os.kill(self.memory_pid, 9)
                    self.memory_pid = None
            elif "Zahlenratespiel starten" in recognized_words:
                # Starte das Numberguessing-Skript und speichere die Prozess-ID (PID)
                numberguessing_process = subprocess.Popen(["python", "numberguessing.py"])

                # Holen der PID des gestarteten Prozesses
                self.numberguessing_pid = numberguessing_process.pid
            elif "Zahlenratespiel beenden" in recognized_words:
                if self.numberguessing_pid:
                    os.kill(self.numberguessing_pid, 9)
                    self.numberguessing_pid = None
            elif "Quiz starten" in recognized_words:
                # Starte das Quiz-Skript und speichere die Prozess-ID (PID)
                quiz_process = subprocess.Popen(["python", "quiz.py"])

                # Holen der PID des gestarteten Prozesses
                self.quiz_pid = quiz_process.pid
            elif "Quiz beenden" in recognized_words:
                if self.quiz_pid:
                    os.kill(self.quiz_pid, 9)
                    self.quiz_pid = None
            elif "Schere Stein Papier starten" in recognized_words:
                # Starte das rockpaperscissors-Skript und speichere die Prozess-ID (PID)
                rockpaperscissors_process = subprocess.Popen(["python", "rockpaperscissors.py"])

                # Holen der PID des gestarteten Prozesses
                self.rockpaperscissors_pid = rockpaperscissors_process.pid
            elif "Schere Stein Papier beenden" in recognized_words:
                if self.rockpaperscissors_pid:
                    os.kill(self.rockpaperscissors_pid, 9)
                    self.rockpaperscissors_pid = None
            elif "Wort Puzzle starten" in recognized_words:
                # Starte das wordpuzzle-Skript und speichere die Prozess-ID (PID)
                wordpuzzle_process = subprocess.Popen(["python", "wordpuzzle.py"])

                # Holen der PID des gestarteten Prozesses
                self.wordpuzzle_pid = wordpuzzle_process.pid
            elif "Wort Puzzle beenden" in recognized_words:
                if self.wordpuzzle_pid:
                    os.kill(self.wordpuzzle_pid, 9)
                    self.wordpuzzle_pid = None
